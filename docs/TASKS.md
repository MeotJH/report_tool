...
- 확인됨: `(Request) => Promise<Response>` 하나로 실제 HTTP 서버에 붙는다. 붙여
  ...
  첫 요청부터 404였다
- 확인됨(2026-09-04): **실제 `node:http` 서버**를 상대로 발행 → 링크 → 열람 → 서명이
  통과한다(`SidecarEndToEnd.test.ts`). 프레임워크 없이 표준 `Request`/`Response`만으로
  붙는다는 주장은 여기서 증명됐다
- 확인 안 됨: Next.js App Router의 `[...path]` 파일 규칙에 붙였을 때. 다만 그것은
  **Next.js 쪽 파일 이름 규칙**이지 우리 코드가 아니다. 실질적인 위험이던 마운트 경로
  문제는 `basePath`로 닫혔다. 호스트가 Node가 아닐 수 있다는 것이 확인되면서
  ([HOST_API.md](HOST_API.md)) 이 항목의 중요도는 내려갔다
