...
- [x] **T57-Y 사용자가 데이터 필드를 직접 정의**: 데이터 패널에서 정적 값(상수),
  데이터 필드, 배열을 추가·삭제한다. 배열은 자식 구성을 정하면 문서에 놓는 순간
  그 구성이 표의 열이 된다. 상수는 `{{const.이름}}`으로 텍스트·표 셀·필드 바인딩에서
  모두 참조되며, `Template.resolveData()` 한 지점에서 합쳐지므로 PDF와 편집기가
  같은 값을 본다. 정의되지 않은 경로를 참조하는 요소는 편집기가 경고로 표시한다.
