...
- 2026-08-23: 호스트 스키마 제거 + N00~N02 완료.
  - `DataProvider.fields()`와 `FieldSchema`를 없애고 데이터 목록의 근거를 템플릿
    `variables` 하나로 통일했다. 팔레트의 출처 배지와 "지울 수 없는 항목"이 사라졌다.
  - 이때 `sensitive`의 유일한 출처가 없어져 민감 필드 마스킹 자동 제안이 함께 빠졌다.
    되살리는 작업을 N07로 등록했다.
  - 실제 임금명세서·비밀유지서약서를 만들어 보고 공백 33건을 `PAYSLIP_GAP_ANALYSIS.md`에,
    실행 단위 N00~N19를 `TASKS_NEXT.md`에 적었다.
  - N00: `TableColumnFitter`가 "열 너비 합 = 표 프레임 너비"를 불변으로 지킨다.
    이전에는 `＋ 열 추가`가 폭을 그냥 붙여 새 열이 표 밖에 생겼고, 편집기가 요소를
    프레임으로 찾으므로 그 열에는 데이터를 연결할 수 없었다.
  - N01·N02: `PdfTextLayout`을 `core/domain/text/TextLayout`으로 옮기고 문단(`\n`)
    분리를 넣었다. 캔버스는 Konva의 `wrap` 대신 이 계산 결과만 그린다. 폭 측정만
    각자 한다(PDF는 임베딩 폰트, 캔버스는 `CanvasTextMeasurer`).
  - 넘치는 문구를 캔버스가 조용히 지우고 PDF가 프레임 밖에 그리던 동작을 없애고,
    양쪽 다 넘쳐 보이게 둔 뒤 `TemplateIssueFinder`가 경고하게 했다.
  - 검증: 서약서 제1조 각 호가 PDF에서 1·2·3·4 순서로 겹침 없이 나오고(이전 3→1→4→2),
    캔버스도 같은 5줄을 그리며, 편집기가 `문구 5줄이 요소 높이보다 길다 (29mm 필요)`를
    표시한다.
  - `npm test`: 테스트 파일 63개, 테스트 425개 통과. core·renderer·designer 타입 검사 통과.
- 2026-08-23: N03 표 셀 2단 해석 완료.
  - `TableCellResolver`가 열 표현식으로 행에서 값을 꺼낸 뒤, 그 값이 다시 표현식이면
    문서 데이터로 채운다. 치환은 두 번까지만 한다.
  - 이 계산을 캔버스·PDF·`UsedCharCollector` 세 곳이 각자 하고 있었다. 폰트 수집이
    빠지면 치환된 한글이 서브셋에 없어 빈칸으로 발행되므로 한 클래스로 모았다.
  - `TemplateReferences`가 정적 셀 안의 표현식 경로도 수집한다. 오타가 다른 참조와
    똑같이 "정의되지 않은 데이터 경로다" 경고로 잡힌다.
  - 정적 표가 설계 모드에서 저장된 행 대신 `⟨key⟩` 한 줄만 보여 주던 것을 고쳤다.
    행이 3개여도 화면에는 1줄이라 무엇을 편집하는지 알 수 없었다. 다만 셀에 적은
    표현식은 값으로 바꾸지 않는다 — 캔버스와 셀 입력기가 같아야 한다.
  - 검증: `기본급 | {{pay.base}}` 표가 PDF에서 `기본급 | 4200000`으로 나오고,
    같은 표에 섞은 순수 고정 문구 행은 그대로 유지된다. 설계 모드에서 정적 행 3개가
    각각 그려지는 것을 Konva 씬 그래프로 확인했다(이전에는 1줄).
  - `npm test`: 테스트 파일 65개, 테스트 438개 통과. 세 패키지 타입 검사 통과.
