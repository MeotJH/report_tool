...
describe("변수 편집 행동", () => {
  ...
  it("이름이 겹치면 추가하지 않고 알린다", () => {
    const { controller, actions } = createEditor();
    actions.addVariable(new ConstantVariable("회사명", "가"));

    actions.addVariable(new ConstantVariable("회사명", "나"));

    expect(controller.getTemplate().variables).toHaveLength(1);
    expect(controller.getNotice()).toContain("회사명");
  });

  it("변수 하나만 교체하고 나머지 순서를 유지한다", () => {
    const { controller, actions } = createEditor();
    actions.addVariable(new ConstantVariable("회사명", "구값"));
    actions.addVariable(new DataVariable("pay.bonus", "상여금", "currency"));

    actions.updateVariable("회사명", new ConstantVariable("회사명", "새값"));

    const variables = controller.getTemplate().variables;
    expect((variables[0] as ConstantVariable).value).toBe("새값");
    expect(variables[1]?.name).toBe("pay.bonus");
  });
  ...
  it("배열 변수를 자식 구성까지 저장한다", () => {
    ...
    actions.addVariable(new DataVariable("deductionItems", "공제 항목", "array", false, [
